...
const int DOOR_A_OPEN_ANGLE = 180;
const int DOOR_B_OPEN_ANGLE = 0;

// Non-blocking motion. One degree every 15 ms is about 1.35 s
// for a full 90-degree gate movement.
const unsigned long SERVO_STEP_INTERVAL = 7;
...
void startGateCycle() {
  ...
  Serial.println("GATE OPENING: A 90->180, B 90->0 (SYNC)");
}
...
void pollRFID() {
  ...
  // APPROVED and WARNING members are both allowed through.
  // DENIED and UNKNOWN cards never start the gate.
  if (record->status == STATUS_APPROVED ||
